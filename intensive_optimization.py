#!/usr/bin/env python3
"""
Intensive optimization study targeting <10 km position RMS and <5 arcsec measurement RMS.
"""
...
def create_high_quality_observations(t0, t1, noise_level=0.0001):
    """Create high-quality observations for better results."""
    # Create observations
    station_ecef = np.array([6378136.3, 0.0, 0.0])  # Greenwich
    t_obs = np.linspace(t0, t1, 12)  # More observations
...
def test_configuration_intensive(L, lam_f, lam_th, N_colloc, t0, t1, obs, t_obs, station_eci):
    """Test a single configuration with intensive evaluation."""
    try:
        ...
        meets_position_target = position_rms < 10.0
        meets_measurement_target = measurement_rms < 5.0
...
def intensive_optimization_study():
    """Run intensive optimization study targeting <10 km position and <5 arcsec measurement."""
    print("=== INTENSIVE ELM OPTIMIZATION STUDY ===")
    ...
    t_obs, obs, station_eci = create_high_quality_observations(t0, t1, noise_level=0.00005)
    print(f"✓ Created {len(t_obs)} observations with {0.00005*180/np.pi*3600:.2f} arcsec noise")
    
    # Define intensive parameter space
    print("3. Testing intensive parameter combinations...")
    
    # Focus on configurations most likely to meet targets
    configurations = [
        # Very strong measurement weights with moderate networks
        (16, 1.0, 10000.0, 30),
        (20, 1.0, 10000.0, 35),
        (24, 1.0, 10000.0, 40),
        (28, 1.0, 10000.0, 45),
        ...
        # Very strong measurement weights with larger networks
        (40, 1.0, 10000.0, 60),
        (48, 1.0, 10000.0, 70),
        (56, 1.0, 10000.0, 80),
        (64, 1.0, 10000.0, 90),
        
        # Extreme measurement weights with larger networks
        (40, 1.0, 50000.0, 60),
        (48, 1.0, 50000.0, 70),
        (56, 1.0, 50000.0, 80),
        (64, 1.0, 50000.0, 90),
        ...
        # Balanced but strong measurement weights
        (24, 1.0, 1000.0, 40),
        (32, 1.0, 1000.0, 50),
        (40, 1.0, 1000.0, 60),
        (48, 1.0, 1000.0, 70),
        
        # High collocation points
        (24, 1.0, 10000.0, 80),
        (32, 1.0, 10000.0, 100),
        (40, 1.0, 10000.0, 120),
        (48, 1.0, 10000.0, 140),
    ...
    print(f"   Testing {len(configurations)} intensive configurations...")
    print()
    ...
    for i, (L, lam_f, lam_th, N_colloc) in enumerate(configurations):
        ...
        result = test_configuration_intensive(L, lam_f, lam_th, N_colloc, t0, t1, obs, t_obs, station_eci)
    ...
    optimization_results = {
        ...
        'metadata': {
            ...
            'noise_level_arcsec': 0.00005*180/np.pi*3600,
            'position_target_km': 10.0,
            'measurement_target_arcsec': 5.0
        }
    }
    
    with open('data/intensive_optimization.json', 'w') as f:
        json.dump(optimization_results, f, indent=2)
    
    print("✓ Results saved to: data/intensive_optimization.json")
    ...
    print()
    print("=== INTENSIVE OPTIMIZATION STUDY COMPLETE ===")
    if target_achievers:
