#!/usr/bin/env python3
"""
Ultra-focused optimization study targeting <5 arcsec measurement RMS with extreme weights.
"""
...
def test_ultra_configuration(L, lam_f, lam_th, N_colloc, t0, t1, obs, t_obs, station_eci):
    """Test a single configuration with ultra-focused evaluation."""
    try:
        # Train ELM with ultra-tight tolerances
        ...
        # Evaluate solution with ultra-high resolution
...
def ultra_focused_study():
    """Run ultra-focused optimization study targeting <5 arcsec measurement RMS."""
    print("=== ULTRA-FOCUSED ELM OPTIMIZATION STUDY ===")
    print("Target: <5 arcsec measurement RMS with extreme weights")
    print()
    ...
    # Create ultra-high-quality observations
    print("2. Creating ultra-high-quality observations...")
    t_obs, obs, station_eci = create_ultra_high_quality_observations(t0, t1, noise_level=0.00001)
    print(f"✓ Created {len(t_obs)} observations with {0.00001*180/np.pi*3600:.2f} arcsec noise")
    
    # Define ultra-focused parameter space - extreme measurement weights
    print("3. Testing ultra-focused parameter combinations...")
    
    configurations = [
        # Ultra-high measurement weights with best networks
        (32, 1.0, 2000000.0, 50),
        (40, 1.0, 5000000.0, 60),
        (48, 1.0, 10000000.0, 70),
        (56, 1.0, 20000000.0, 80),
        (64, 1.0, 50000000.0, 90),
        
        # Even more extreme weights
        (32, 1.0, 10000000.0, 50),
        (40, 1.0, 20000000.0, 60),
        (48, 1.0, 50000000.0, 70),
        (56, 1.0, 100000000.0, 80),
        (64, 1.0, 200000000.0, 90),
        
        # Maximum weights with high collocation
        (32, 1.0, 50000000.0, 100),
        (40, 1.0, 100000000.0, 120),
        (48, 1.0, 200000000.0, 140),
        (56, 1.0, 500000000.0, 160),
        (64, 1.0, 1000000000.0, 180),
    ]
    
    print(f"   Testing {len(configurations)} ultra-focused configurations...")
    print()
    ...
    for i, (L, lam_f, lam_th, N_colloc) in enumerate(configurations):
        ...
        result = test_ultra_configuration(L, lam_f, lam_th, N_colloc, t0, t1, obs, t_obs, station_eci)
    ...
    print(f"  Position RMS: {best_measurement['position_rms']:.2f} km")
    ...
    optimization_results = {
        ...
        'metadata': {
            ...
            'noise_level_arcsec': 0.00001*180/np.pi*3600,
            'position_target_km': 10.0,
            'measurement_target_arcsec': 5.0
        }
    }
    
    with open('data/ultra_focused_optimization.json', 'w') as f:
        json.dump(optimization_results, f, indent=2)
    
    print("✓ Results saved to: data/ultra_focused_optimization.json")
    ...
    print()
    print("=== ULTRA-FOCUSED OPTIMIZATION STUDY COMPLETE ===")
    if target_achievers:
        print("🎉 SUCCESS: Found configurations meeting both targets!")
    else:
        print("⚠️  No configurations met both targets.")
        if best_measurement['measurement_rms'] < 5.0:
            print("🎯 SUCCESS: Achieved measurement target!")
        if best_position['position_rms'] < 10.0:
            print("🎯 SUCCESS: Achieved position target!")
